...
int sem_id = semget(1111, 6, 0200 | IPC_CREAT);
...
if((sem = semctl(sem_id, 3, SETVAL, 1)) == -1){
        perror("semctl error (program)");
        exit(EXIT_FAILURE);
}

if((sem = semctl(sem_id, 4, SETVAL, 1)) == -1){
        perror("semctl error (program)");
        exit(EXIT_FAILURE);
}

if((sem = semctl(sem_id, 5, SETVAL, total)) == -1){
        perror("semctl error (program)");
       exit(EXIT_FAILURE);
}

shm_id1 = shmget(2221, 1, 0606 | IPC_CREAT);
if(shm_id1 == -1){
	perror("shmget error (program)");
	exit(EXIT_FAILURE);
}

shm_id2 = shmget(2222, 1, 0606 | IPC_CREAT);
if(shm_id2 == -1){
        perror("shmget error (program)");
        exit(EXIT_FAILURE);
}

shm_id3 = shmget(2223, 1, 0606 | IPC_CREAT);
if(shm_id2 == -1){
        perror("shmget error (program)");
        exit(EXIT_FAILURE);
}
...
for(int i = 0; i < amount_pi; i++){
	...
	case -1: if(errno == EAGAIN){
		 perror("fork error_eagain (program)");
		 exit(EXIT_FAILURE);
		}
		perror("fork error (program)");
		exit(EXIT_FAILURE);

	case 0: if(execlp("./pi","pi", argv[3], NULL) == -1){
...
if((shm_rm = shmctl(shm_id3, IPC_RMID, 0)) == -1){        perror("shmctyl error (program)");
        exit(EXIT_FAILURE);
}
