...
int main()
{
    ...
    if(file == NULL) {
        cout << "Nie mozna otworzyc pliku (producent)";
        exit(-1);
    }
    ...
    int odlaczenie3 = shmdt(adres);
        if(odlaczenie3 == -1) {
        cout << "Problem z odlaczeniem pamieci dzielonej (producent)";
        exit(EXIT_FAILURE);
    }
...
static void get_key(void)
{
    ...
    if(key_sem == -1)
    {
        cout << "Blad tworzenia klucza (producent)" << endl;;
        exit(-1);
    }
}
...
static void get_key_shm(void)
{
    ...
    if(key_shm == -1)
    {
        cout << "Blad tworzenia klucza pamieci (producent)" << endl;
        exit(-1);
    }
}
